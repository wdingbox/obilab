...
public function get_img_src_cur_dir(){
		...
		$this->img_path = "../../../../../___bigdata/___compact/___solid/";
	}
public function img_src_of($key, $val ){
   if( "bid"==$key ){
      return $this->img_path . "odb/bronze/0000-shang/".$val.".jpg";  
   }
...
public function htm_tr_dat_edit($row) {
		...
		foreach($row as $key => $val ) {
			...
			case 'htoj':
            case 'hink':
            case 'jgroots'://for Shange 
				echo $this->htm_imga($key, $val);
				break;
...
public function get_Append_Params($sql, $row, $key) {
      ...
      if ( stripos($sql, "bronze0000shang", $pos+4) > 0 ) {
			if ( "jink"==$key) {								
				if( !isset($row['picname']) ){
					//echo "@@@@";
					//print_r($row);
					//echo "@@@@<br>";
					$row['picname']="no_picname";
				}
				return "bronze0000shang,picname,".$row['picname'].",$key";//dbname, primarykey, primaryval, appendkey
				return "UPDATE bronze0000shang SET $key = ppp WHERE picname = ".$row['picname']."";
			}
		};
		...
		foreach($row as $key => $val ) {
         ...
         case 'jink3':
            ...
            case 'jgroots':
				...
				echo $this->htm_divaimg($key, $val, $pars);
				break;
			case 'picname':
				echo "<td pars=''><div>";
