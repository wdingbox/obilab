...
<head>
    ...
    <script type="text/javascript" src="../_js/uti.js"></script>
     ...
     <script type="text/javascript" src="D3Uti.js"></script>
...
$(document).ready(function(){
    //$(".tmpimg").attr("title","c : copy zoom to current;\n n : next\n z : toggle zoom.");
            ...
            $("#formsubmit").submit(function(){
                var sql = $("#sql").val();
                svrSelectOptiontsHistorySave(sql);
                //alert(sql.length);
...
<form method="get" action="<?php echo basename(__FILE__);?>" id="formsubmit">
   ...
   <select id="sss" name="sss"  style="max-width:20px;">
      ...
      ?>
      <optgroup label="History" style="color:#000000; background-color:#aa1212">
        <?php atpl::printent("db_view_selectoptions_History.tpl"); ?>
      </optgroup>
   </select>
...
<option id="a">===</option>
<option value="ooTree_json">d3:ooTree_json</option>
<option value="ooLink_json">d3:ooLink_json</option>
</select>
<br/>
...
echo "<table border='1'>\r\n";
   ...
   foreach ($ret as $row){
      ...
      $totFreq += intval ($row['freq']);
      if(isset($row['jtoh'])) {
         $r2htm->push2TmpArr($row['jtoh']);
      }
   }
echo "</table>\r\n";
echo "<br>totFreq=$totFreq";
